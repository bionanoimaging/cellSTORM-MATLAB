...
if(~exist('filename_CSV'))
    ...
    % mkdir if not exist
    filedir_dest = ['./images/' filename_TIF '/'];
...
% define localization grid 
max_x = max(DataFrame(:, 3));
max_y = max(DataFrame(:, 4));
mysize = upsampling_factor*[M N];
...
for(frmNum = 1:nframes)
    ...
    iframe_video = imresize(y_video,upsampling_factor,'nearest'); % imresize(y,upsampling_factor,'box');
    ...
    % read the datapoints from Thudnerstorm 
    xevents_i = DataFrame(:, 4);
    yevents_i = DataFrame(:, 3);
    ...
    mygt_loc_psf = conv2(SpikesImage,psfHeatmap,'same');
    ...
    if(1), dip_image(cat(3, mygt_loc_psf, iframe_video)), end
    
    %% Divide image into patches and save it
    patchsize = 256;
    
    patchnum_m = 1:(round(size(iframe_gt,1))/patchsize)-1;
    patchnum_n = 1:(round(size(iframe_gt,2))/patchsize)-1;
    ...
    for im = patchnum_m
        for in = patchnum_n
            % divide into patches
            iframe_gt_patch = iframe_gt(im*patchsize:(im+1)*patchsize-1, in*patchsize:(in+1)*patchsize-1);
            iframe_video_patch = iframe_video(im*patchsize:(im+1)*patchsize-1, in*patchsize:(in+1)*patchsize-1);
            spike_patch = SpikesImage(im*patchsize:(im+1)*patchsize-1, in*patchsize:(in+1)*patchsize-1);
            iframe_gt_ts_patch = iframe_gt_ts(im*patchsize:(im+1)*patchsize-1, in*patchsize:(in+1)*patchsize-1);
