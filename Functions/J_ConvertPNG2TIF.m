...
myfiledir = '/media/useradmin/Data/Benedict/Dropbox/Dokumente/Promotion/PROJECTS/STORM/MATLAB/LINES/';
...
srcFiles = dir([filedir '*outputs.png']);  % the folder in which ur images exists
iframe = {};
...
if(1)
    %% Read the orignial data
    srcFiles = dir([filedir '*targets.png']);  % the folder in which ur images exists
    iframe = {};
    for i = 1 : length(srcFiles)
        filename = strcat(filedir,srcFiles(i).name);
        iframe{i}  = imread(filename);
